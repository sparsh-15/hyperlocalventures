          ...
          {navLinks.map((link) =>
            link.children ? (
              <div key={link.label} className="relative" ref={propositionRef}>
                ...
                {propositionOpen && (
                  <div className="absolute left-0 top-full z-50 mt-1.5 w-56">
                    <div className="glass-strong rounded-2xl border border-primary/10 bg-white/95 p-3 shadow-xl backdrop-blur-md">
              ...
              <a
                key={link.href}
                href={link.href}
                className="text-sm font-medium text-muted-foreground transition-colors duration-300 hover:text-primary relative after:absolute after:bottom-[-4px] after:left-0 after:h-px after:w-0 after:bg-primary after:transition-all after:duration-300 hover:after:w-full"
              >
                {link.label}
              </a>
            )
              ...
              {navLinks.map((link) =>
                link.children ? (
                  <div key={link.label} className="flex flex-col gap-1">
                    ...
                    {link.children.map((child) => (
                      <a
                        ...
                        className="pl-3 text-base text-muted-foreground transition-colors hover:text-primary"
                      >
                        {child.label}
                      </a>
                    ))}
                  ...
                  <a
                    ...
                    href={link.href}
                    onClick={() => setMobileOpen(false)}
                    className="text-base text-muted-foreground transition-colors hover:text-primary py-1"
                  >
                    {link.label}
                  </a>
                )
